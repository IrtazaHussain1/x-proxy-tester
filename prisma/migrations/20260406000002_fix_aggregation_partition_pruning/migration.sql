-- Migration: Fix aggregation query so MySQL can prune partitions
--
-- PROBLEM:
--   WHERE DATE(pr.timestamp) = summary_day
--   wraps the partition key column in a function — MySQL cannot prune partitions
--   and scans ALL partitions instead of just the target month.
--
-- FIX:
--   WHERE pr.timestamp >= summary_day AND pr.timestamp < summary_day + INTERVAL 1 DAY
--   MySQL sees the bounds directly and prunes to a single partition.
--
-- APPLY WITH:
--   npx prisma db execute --file prisma/migrations/20260406000002_fix_aggregation_partition_pruning/migration.sql --schema prisma/schema.prisma
...
BEGIN
  ...
  SELECT
    dt.day, dt.proxy_id, dt.location, dt.relay_server_id, dt.relay_server_ip,
    dt.total_requests, dt.success_count, dt.failure_count, dt.success_rate_pct,
    dt.avg_response_time_ms, dt.min_response_time_ms, dt.max_response_time_ms,
    dt.p50_response_time_ms, dt.p95_response_time_ms, dt.p99_response_time_ms,
    dt.timeout_count, dt.connection_error_count, dt.http_error_count, dt.dns_error_count,
    dt.rotation_count,
    dt.avg_download_speed_mbps, dt.avg_upload_speed_mbps,
    dt.max_download_speed_mbps, dt.max_upload_speed_mbps,
    dt.min_download_speed_mbps, dt.min_upload_speed_mbps,
    dt.unique_ips_count, dt.ip_diversity_score
  FROM (
    SELECT
      summary_day                                                             AS day,
      pr.proxy_id,
      ...
      -- Percentiles: range filter on timestamp (not DATE()) so partition pruning works
      (SELECT sub.response_time_ms FROM (
        SELECT response_time_ms,
               ROW_NUMBER() OVER (ORDER BY response_time_ms) AS rn,
               COUNT(*) OVER ()                              AS total
        FROM proxy_requests pr2
        WHERE pr2.timestamp >= summary_day
          AND pr2.timestamp <  summary_day + INTERVAL 1 DAY
      ...
      (SELECT sub.response_time_ms FROM (
        ...
        WHERE pr3.timestamp >= summary_day
          AND pr3.timestamp <  summary_day + INTERVAL 1 DAY
      ...
      (SELECT sub.response_time_ms FROM (
        ...
        WHERE pr4.timestamp >= summary_day
          AND pr4.timestamp <  summary_day + INTERVAL 1 DAY
    ...
    -- Range comparison instead of DATE() — allows MySQL to prune to a single partition
    WHERE pr.timestamp >= summary_day
      AND pr.timestamp <  summary_day + INTERVAL 1 DAY
    GROUP BY pr.proxy_id, p.location, p.relay_server_id, p.relay_server_ip_address
  ) AS dt
